...
const Results = (props: { address?: string } ): JSX.Element => {
    ...
    setLoadingJobs((prevJobs) => {
      ...
      if (newState === 'success') {
        console.log(
          ...
          + `\n%cRun %cwindow.siteSleuth['${job}']%c to inspect the raw the results`,
        ...
        if (!(window as any).siteSleuth) (window as any).siteSleuth = {};
        if (data) (window as any).siteSleuth[job] = data;
      }
  ...
  return (
    <ResultsOuter>
      ...
      {/* { address?.includes(window?.location?.hostname || 'site-sleuth.xyz') && <SelfScanMsg />} */}
      ...
      <FilterButtons>{ showFilters ? <>
        ...
        <span className="toggle-filters" onClick={() => setShowFilters(false)}>Hide</span>
          ...
          <div className="control-options">
            ...
            <a target="_blank" rel="noreferrer" href="https://github.com/dlpra/site-sleuth"><span className="toggle-filters">View GitHub</span></a>
          </div>
      ) }
      </FilterButtons>
      <ResultsContent>
